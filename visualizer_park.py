...
class Visualizer:
  ...
  def node_action(self, nodes, test=False):
    if test:
      ...
      L = [4218,5041,1060,2198,5670,9332,14473,19432,21743]
        ...
        '''
        for n in nodes:
          if n.availible and n.a_counter < 30:
            L.append(n.current_history)
          else:
            L.append('nan')
        '''
        for n in nodes:
  ...
  def init_plot(self):
    ...
    plt.axis([-1000, max(map(lambda x: x.posX, self.nodes))+2000, -2000, max(map(lambda x: x.posY, self.nodes))+3000])
